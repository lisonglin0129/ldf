...
class Template
{
    ...
    protected $config = [
        ...
        'compile_type'       => 'ViewDriver', // 模板编译类型
    ...
    public function __construct(array $config = [])
    {
        ...
        // 初始化模板编译存储器
        $type  = $this->config['compile_type'] ? $this->config['compile_type'] : 'ViewDriver';
        $class = false !== strpos($type, '\\') ? $type : '\\core\\view\\' . ucwords($type);
    ...
    public function parseTagLib(string $tagLib, string &$content, bool $hide = false): void
    {
        if (false !== strpos($tagLib, '\\')) {
            ...
            $className = '\\core\\view\\' . ucwords($tagLib);
        }
